...
def convert2binary(text, encoding='utf-8', errors='Nope. Something went wrong.'):
    bits = bin(int(binascii.hexlify(text.encode(encoding, errors)), 16))[2:]
    return bits.zfill(8 * ((len(bits) + 7) // 8))

def convert2text(bits, encoding='utf-8', errors='Nope. Something went wrong.'):
    n = int(bits, 2)
...
original = input('Please type in your message and then press enter: \n')
